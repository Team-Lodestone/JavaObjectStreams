/** @file PrimitiveJavaObjectParser.cpp
*
* @author Zero_DSRS_VX
* @date 3/25/26
*
...
namespace javaobject::type::primitive::parsers {
    std::shared_ptr<types::IPrimitiveObject> PrimitiveJavaObjectParser::operator()(PrimitiveTypeCodeParser &parser) const {
        return std::make_shared<types::PrimitiveJavaObject>(
            parser.parserStorage().objectParser->readNext()
        );
    }
}
